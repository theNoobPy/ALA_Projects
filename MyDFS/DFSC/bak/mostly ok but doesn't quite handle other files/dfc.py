...
#print(server_conf()[0])#all server_list needs to be is a 2d array of the servers.
...
def client():
	...
	for i in range(len(server_list)):
		...
		try:
			...
			status_s = ('Connected to server', 'DFS'+str(i+1))
			print(status_s[0], status_s[1])
			connected = 1
			time.sleep(1)
		except ConnectionRefusedError:
			status1 = ('Could not connect to DFS server: ', i+1)
			print(status_s[0], status_s[1])
