...
def put(new_dir_path):
    ...
    if not os.path.exists(savepath):                  # check if the path isn't created yet
        os.mkdir(savepath)                            # make dir creates the path if it is not
...
def list_files(username):
    ...
    if file_dir_list == []:
        ...
        if file_list == [[]]:
            ...
            # delete the file 
            send2trash('filenames.txt')
        
        
# gets files from servers
def get(username):
    print('Starting transfer to client...')

    file_name = conn.recv(2048).decode()
    list_of_files_to_send = os.listdir(username+'\\'+file_name)
    parts_max = len(list_of_files_to_send)
    
    part_num = 0
    while 1:
        file_to_send = username+'\\'+file_name+'\\'+list_of_files_to_send[part_num]
        print('Sending:'+file_to_send+' of size '+str(os.stat(file_to_send).st_size)+'KB to client')

        if part_num == parts_max-1:
            send_another_file = 0
        else:
            send_another_file = 1

        command = list_of_files_to_send[part_num]+'[|Delimiter\\_\\Marker|]'+str(os.stat(file_to_send).st_size)+'[|Delimiter\\_\\Marker|]'+str(send_another_file)#the delim marker has \s cause those aren't allowed on file names making it a reliable delimiter.
        ...
        wait_for_complete = conn.recv(1024).decode()
        print(wait_for_complete)
        part_num += 1                  #advance the iterator
        
        if part_num == parts_max:
            break

    print('Transfer Complete')
    send2trash(username+'\\'+file_name)
